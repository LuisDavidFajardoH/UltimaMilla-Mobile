...
const EntregarPedido = ({ route, navigation }) => {
  ...
  }, [id_pedido, navigation, fetchPedidoDetails]);
  ...
  const startDelivery = useCallback(() => {
    if (id_pedido) {
      navigation.navigate('ProcesoPago', { 
        id: id_pedido,  // Pass the ID directly instead of nesting it
        refresh: true 
      });
      console.log('Iniciando entrega para el pedido:', id_pedido);
    } else {
      console.error("Información del pedido no disponible.");
      Alert.alert('Error', 'Información del pedido no disponible');
    }
  ...
  return (
    <Layout style={[styles.container, { paddingTop: insets.top }]}>
      <TopNavigation
        title={`Entregar Pedido #${pedido.ID_pedido}`}
        alignment="center"
